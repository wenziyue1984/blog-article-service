package com.wenziyue.blog.article.biz.mq.listener;
...
import com.wenziyue.blog.article.dal.dto.ArticlePvDTO;
...
import static com.wenziyue.blog.article.common.constants.RedisConstant.*;
import static com.wenziyue.blog.article.common.constants.RocketMqTopic.ArticlePvTopic;
...
//            val userEntity = redisUtils.get(LOGIN_TOKEN_KEY + dto.getToken(), UserEntity.class);
            val loginUser = redisUtils.get(AuthConstants.LOGIN_USER_INFO_KEY_PREFIX + AuthHelper.currentUserId(), LoginUser.class);
            if (loginUser == null) {
                // token失效
                item = BlogUtils.fp(dto.getIp(), dto.getUserAgent());
            } else {
                item = BlogUtils.fp(loginUser.getUserId());
            }
        } else {
            item = BlogUtils.fp(dto.getIp(), dto.getUserAgent());
        }
//        countPvByJava(dto.getArticleId(), item);
        if (!countPvByLua(dto.getArticleId(), item)) {
            log.error("pv统计失败:{}", dto);
        }
    }
