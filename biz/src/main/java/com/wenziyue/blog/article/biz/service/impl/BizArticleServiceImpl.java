...
import com.wenziyue.auth.core.enums.UserRoleEnum;
...
import com.wenziyue.blog.article.common.constants.RedisConstant;
import com.wenziyue.blog.article.common.enums.*;
import com.wenziyue.blog.article.common.exception.BlogResultCode;
import com.wenziyue.blog.article.common.utils.BlogUtils;
import com.wenziyue.blog.article.dal.dto.*;
import com.wenziyue.blog.article.dal.entity.*;
import com.wenziyue.blog.article.dal.service.*;
...
import static com.wenziyue.blog.article.common.constants.RedisConstant.ARTICLE_UPDATE_TIME_KEY;
import static com.wenziyue.blog.article.common.constants.RedisConstant.SLUG_LISTEN_KEY;
import static com.wenziyue.blog.article.common.constants.RocketMqTopic.*;
...
public class BizArticleServiceImpl implements BizArticleService {
    ...
    public String generateSlug(SlugDTO dto) {
        // 加入mq队列
        Long userId = Long.valueOf(AuthHelper.currentUserId());
...
//            redisUtils.delete(listenKey);
    ...
    public String saveOrUpdate(ArticleDTO dto) {
        ...
        if (articleService.count(Wrappers.<ArticleEntity>lambdaQuery()
                ...
                .eq(ArticleEntity::getUserId, AuthHelper.currentUser().getUserId())) > 0) {
        ...
        try {
            ...
            if (dto.getId() == null) {
                ...
                if (!articleEntity.getUserId().toString().equals(AuthHelper.currentUserId())) {
            ...
            articleOperationLogService.save(ArticleOperationLogEntity.builder()
                    ...
                    .articleId(articleEntity.getId())
                    .operatorId(userId)
                    .operatorName(AuthHelper.currentUser().getUsername())
    ...
    public ArticleDTO getArticleDetail(Long id) {
        ...
        if (articleEntity.getStatus().equals(ArticleStatusEnum.HIDDEN)) {
            if (!AuthHelper.currentUser().getRoles().contains(UserRoleEnum.ADMIN.getRole()) && !articleEntity.getUserId().toString().equals(AuthHelper.currentUserId())) {
                throw new ApiException(BlogResultCode.ARTICLE_NOT_EXIST);
            }
        }
        // 查询标签
    ...
    public PageResult<ArticleDTO> pageArticle(ArticlePageDTO dto) {
        ...
        if (dto.getUserId() == null && currentUser.getRoles().equals(UserRoleEnum.USER.getRole())) {
            dto.setUserId(Long.valueOf(currentUser.getUserId()));
        }
        if (currentUser.getRoles().equals(UserRoleEnum.USER.getRole()) && !currentUser.getUserId().equals(dto.getUserId().toString())) {
    ...
    public void hideArticle(Long id) {
        // 管理员以及用户自己可以隐藏文章
        val currentUser = AuthHelper.currentUser();
        if (!currentUser.getRoles().equals(UserRoleEnum.ADMIN.getRole())) {
            throw new ApiException(BlogResultCode.USER_NO_PERMISSION);
        }
        val articleEntity = articleService.getById(id);
        if (articleEntity == null) {
        ...
        if (!currentUser.getUserId().equals(articleEntity.getUserId().toString())) {
            throw new ApiException(BlogResultCode.USER_NO_PERMISSION);
        }
        if (articleEntity.getStatus().equals(ArticleStatusEnum.HIDDEN)) {
            return;
    ...
    public void deleteArticle(Long id) {
        ...
        if (!currentUser.getRoles().equals(UserRoleEnum.ADMIN.getRole())
                && !currentUser.getUserId().equals(articleEntity.getUserId().toString())) {
            throw new ApiException(BlogResultCode.USER_NO_PERMISSION);
        }
    ...
    public void likeArticle(Long articleId) {
        ...
        // 不能点赞自己的文章
        if (articleEntity.getUserId().toString().equals(userId)) {
...
//        redisUtils.xAdd(RedisConstant.ARTICLE_LIKE_STREAM_KEY, map);
    ...
    public void favoritesArticle(FavoritesArticleDTO dto) {
        ...
        val faEntity = faService.getOne(Wrappers.<FavoritesArticleEntity>lambdaQuery()
                ...
                .eq(FavoritesArticleEntity::getUserId, AuthHelper.currentUserId()));
        if (faEntity != null) {
//            throw new ApiException(BlogResultCode.FAVORITES_ARTICLE_EXIST);
    ...
    public void cancelFavoritesArticle(FavoritesArticleDTO dto) {
        ...
        faService.cancelFavoritesArticle(dto.getFavoritesFolderId(), dto.getArticleId(), Long.valueOf(AuthHelper.currentUserId()));
    }
